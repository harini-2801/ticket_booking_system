...
import { LayoutDashboard, Plus, DollarSign, Ticket, Users, Calendar, PlusCircle, RefreshCw, X } from 'lucide-react';
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          <h1 className="text-3xl font-extrabold text-white flex items-center gap-2">
            <LayoutDashboard className="w-7 h-7 text-blue-500" /> Organiser Dashboard
          </h1>
          <p className="text-slate-400 text-sm mt-1">Manage event listings, set category prices, and track revenues.</p>
        </div>

        <button
          onClick={() => setShowCreateModal(true)}
          className="bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm px-4 py-2.5 rounded-xl shadow-lg transition-colors flex items-center justify-center gap-2"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl flex items-center justify-between">
          ...
          <div className="p-3 bg-emerald-950/60 border border-emerald-800 rounded-xl text-emerald-400">
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Total Confirmed Tickets</p>
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Active Event Listings</p>
            <h3 className="text-2xl font-extrabold text-purple-400 mt-1">{stats.totalEvents}</h3>
          </div>
          <div className="p-3 bg-purple-950/60 border border-purple-800 rounded-xl text-purple-400">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        <div className="p-6 border-b border-slate-800">
          <h3 className="text-lg font-bold text-white">Event Sales & Revenue Summary</h3>
        </div>

        {loading ? (
          <div className="p-12 text-center text-slate-400">
            <RefreshCw className="w-6 h-6 animate-spin mx-auto mb-2 text-blue-500" /> Loading events...
          </div>
        ) : summary.length === 0 ? (
          <div className="p-12 text-center text-slate-400 space-y-3">
            <p>No event listings created yet.</p>
            <button
              onClick={() => setShowCreateModal(true)}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-950 text-[11px] font-semibold uppercase tracking-wider text-slate-400 border-b border-slate-800">
                ...
                {summary.map((event) => (
                  <tr key={event.id} className="hover:bg-slate-800/50 transition-colors">
                    <td className="p-4 font-bold text-white">
                      <div>{event.title}</div>
                      <span className="text-[10px] text-blue-400 font-semibold uppercase">{event.category}</span>
                    </td>
                    ...
                    <td className="p-4">
                      ...
                      <span className="text-[10px] text-slate-400">
                        {event.bookedSeatsCount} / {event.totalSeats} seats booked
                      </span>
                    </td>
                    <td className="p-4">
                      <span className="px-2.5 py-1 rounded-md bg-amber-950/60 border border-amber-800 text-amber-300 font-bold">
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 space-y-6 shadow-2xl relative">
            ...
            <h3 className="text-xl font-extrabold text-white">Create New Event Listing</h3>
            ...
            <form onSubmit={handleCreateEvent} className="space-y-4 text-xs">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Inception IMAX 3D"
